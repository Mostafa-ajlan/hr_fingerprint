...
class HrFingerprintDevice(models.Model):
    ...
    @api.model
    def render_and_send(self, device, action, params, websocket=True):
        print(device,"devicedevicedevicedevice")
        print(action,"actionactionactionaction")
        print(params,"paramsparamsparamsparams")
        iot_box = self.env["iot.box"].search([("id", "=", device["iot_id"][0])])
        iot_box.write({'is_websocket_active': True})

        print(iot_box.is_websocket_active,"is_websocket_activeis_websocket_active")
        
        self._send_websocket({
            "iotDevice": {
                "iotIdentifiers": [self.env["iot.box"].search([("id", "=", device["iot_id"][0])]).identifier],
                "identifiers": [{
    ...
    def _send_websocket(self, message):
        ...
        """
        print("_send_websocket_send_websocket")
